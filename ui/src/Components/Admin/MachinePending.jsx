...
export default function MachinePending() {
    ...
    useEffect(() => {
        setStatus(status);
        fetch(`http://127.0.0.1:8000/api/machine_status/${status}`, { method: 'GET' })
            .then(response => response.json())
            .then(data => {
                setMachineRequests(data);
                console.log(data); setDataCount(data.length)
            })
    ...
    const showData = () => {
        ...
        const currentLandDetails = machinerequests.slice(indexOfFirstPage, indexOfLastPage);
        try {
            return currentLandDetails.map((item, index) => (
                <tbody key={index}>
                    <tr style={{ lineHeight: "40px" }}>
                        <td>{postsPerPage * (currentPage - 1) + index + 1}</td>
                        <td ><Link to={`/handlemachinerequests/${item.machine_id}/${item.id}`} className='text-decoration-none text-danger'><MdOutlineDescription /></Link></td>
    ...
    const showPagination = () => {
        const pageNumbers = [];
        const totalLandDetails = machinerequests.length;
        ...
        <div className="container p-4">
            <div className='card  text-decoration-none w-100 mx-auto m-5 ps-3 pe-3 rounded-1 shadow-lg'>
                ...
                {machinerequests.length > 0 ? (
                    ...
                    (<div className='p-3 m-3  fw-bold text-danger text-center fs-4 mt-3 mx-auto w-50'>No Data Available....</div>)
                }
            </div>
