...
function MachineDetails() {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/machine_details/${admin_id}/`, { method: 'GET' })
      .then(response => response.json())
      .then(data => { setMachineDetails(data); setDataCount(data.length) })
  ...
  const handleDelete = async () => {
    try {
      if (checkedRows.length > 0) {
        ...
        if (result === true) {
          const admin_id = localStorage.getItem('id');
          const response = await fetch(`http://127.0.0.1:8000/api/delete_machine_details?admin_id=${admin_id}`, {
            method: 'DELETE',
            headers: {
  ...
  const handleCheckAll = () => {
    if (checkedRows.length === machinedetails.length) { setCheckedRows([]) }
    else { setCheckedRows(machinedetails.map(item => item.id)) }
  }

  const checkAllRows = checkedRows.length === machinedetails.length;


  const showData = () => {
    const indexOfLastPage = currentPage * postsPerPage;
    const indexOfFirstPage = indexOfLastPage - postsPerPage;
    const currentLandDetails = machinedetails.slice(indexOfFirstPage, indexOfLastPage);
      ...
      return currentLandDetails.map((item, index) => (
        <tbody key={index}>
          <tr style={{ lineHeight: "40px" }}>
            ...
            <td style={checkedRows.includes(item.id) ? { backgroundColor: "aliceblue" } : {}}><Link to={`/machinedetails/${item.id}`}><MdOutlineDescription /></Link></td>
            <td style={checkedRows.includes(item.id) ? { backgroundColor: "aliceblue" } : {}}>{item.machine_name}</td>
            <td style={checkedRows.includes(item.id) ? { backgroundColor: "aliceblue" } : {}}>{item.owner_name}</td>
  ...
  const showPagination = () => {
    const pageNumbers = [];
    const totalLandDetails = machinedetails.length;
      ...
      {machinedetails.length > 0 ? (
        <div className='card text-decoration-none w-100 mx-auto m-5 ps-3 pe-3 rounded-1 shadow-lg'>
          <div className='d-flex justify-content-between w-100 mx-auto rounded-1 border' style={{ marginTop: '-30px', backgroundColor: "orange" }}>
            ...
            <div className='p-3 text-center text-light fs-5 fw-bold'>Machine Details&nbsp;<SlEnergy className='text-danger' />{datacount}</div>
            ...
            <table className="table align-items-center justify-content-center mb-0">
              <thead>
                <tr >
                  ...
                  <th className='text-primary'>Machine Details</th>
                  <th className='text-primary'>Machine Name</th>
                  <th className='text-primary'>Owner Name</th>
