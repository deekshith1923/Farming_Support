...
export default function LandRequests() {
    ...
    useEffect(() => {
        setStatus(status);
        fetch(`http://127.0.0.1:8000/api/land_status/${status}`, { method: 'GET' })
            .then(response => response.json())
            .then(data => { setLandRequests(data); setDataCount(data.length) })
    ...
    const showData = () => {
        ...
        const currentLandDetails = landrequests.slice(indexOfFirstPage, indexOfLastPage);
        try {
            return currentLandDetails.map((item, index) => (
                <tbody key={index}>
                    <tr style={{ lineHeight: "40px" }}>
                        ...
                        <td >{item.address}</td>
                        <td >{item.mobile}</td>
                        <td ><Link to={`/handlelandrequests/${item.land_id}/${item.id}`} className='text-decoration-none text-danger'><MdOutlineDescription /></Link></td>
                        <td >{convertToLocalDate(item.start_date)}</td>
                        <td >{convertToLocalDate(item.end_date)}</td>
                        <td >{item.cost}</td>
                        <td >{item.status}</td>
                        {/* <td ><button className='btn btn-success w-100'> */}
                        {/* <td ><button className={`${status === 'Pending' ? '' : 'hidden'} btn btn-danger w-100`} onClick={(e) => handleRequests(item.id, "Rejected")}><FaRegThumbsDown className='text-light' /> </button></td> */}
                    </tr>
                </tbody>
            ));
        } catch (e) {
    ...
    const showPagination = () => {
        const pageNumbers = [];
        const totalLandDetails = landrequests.length;
        ...
        <div>
            <div className='card  text-decoration-none w-100 mx-auto m-5 ps-3 pe-3 rounded-1 shadow-lg'>
                <div className=' d-flex justify-content-between rounded-1 border' style={{ marginTop: '-30px', backgroundColor: "orange" }}>
                    ...
                    <div className='p-3  text-light fs-5 fw-bold'>Land Requests&nbsp;<SlEnergy className='text-dark' />{datacount}</div>
                ...
                {landrequests.length > 0 ? (
                        ...
                        <table className="table align-items-center justify-content-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th className='text-primary'>Requested Land</th>
                    ...
                    (<div className='p-5 m-3 fw-bold text-danger text-center fs-4 mt-3 mx-auto w-50'>No Data Available....</div>)
                }
            </div>
