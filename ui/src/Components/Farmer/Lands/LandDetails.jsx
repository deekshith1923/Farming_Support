...
export default function LandDetails() {
    ...
    useEffect(() => {
        ...
        fetch(`http://127.0.0.1:8000/api/land_details/${farmer_id}/`, { method: 'GET' })
            .then(response => response.json())
            .then(data => { setLandDetails(data); setDataCount(data.length) })
    ...
    const handleDelete = async () => {
        try {
            if (checkedRows.length > 0) {
                ...
                if (result === true) {
                    const farmer_id = localStorage.getItem('id');
                    const response = await fetch(`http://127.0.0.1:8000/api/delete_lands_details?farmer_id=${farmer_id}`, {
                        method: 'DELETE',
                        headers: {
    ...
    const handleCheckAll = () => {
        if (checkedRows.length === landDetails.length) { setCheckedRows([]) }
        else { setCheckedRows(landDetails.map(item => item.id)) }
    }

    const checkAllRows = checkedRows.length === landDetails.length;


    const showData = () => {
        const indexOfLastPage = currentPage * postsPerPage;
        const indexOfFirstPage = indexOfLastPage - postsPerPage;
        const currentLandDetails = landDetails.slice(indexOfFirstPage, indexOfLastPage);
            ...
            return currentLandDetails.map((item, index) => (
                <tbody key={index}>
                    <tr style={{ lineHeight: "40px" }}>
                        ...
                        <td style={checkedRows.includes(item.id) ? { backgroundColor: "aliceblue" } : {}} ><Link to={`/handleland/${item.id}`} className='text-decoration-none text-danger'><MdOutlineDescription /></Link></td>
    ...
    const showPagination = () => {
        const pageNumbers = [];
        const totalLandDetails = landDetails.length;
            ...
            {landDetails.length > 0 ? (
                <div className='card text-decoration-none w-100 mx-auto m-5 ps-3 pe-3 rounded-1 shadow-lg'>
                    <div className='d-flex justify-content-between w-100 mx-auto rounded-1 border' style={{ marginTop: '-30px', backgroundColor: "orange" }}>
                        ...
                        <div className='p-3 text-center text-light fs-5 fw-bold'>Land Details&nbsp;<SlEnergy className='text-danger' />{datacount}</div>
                        ...
                        <table className="table align-items-center justify-content-center mb-0">
                            <thead>
                                <tr >
                                    ...
                                    <th className='text-primary'>Land Detaills</th>
