...
export default function MachineReqStatus() {
    const [machinerequested, setMachineRequested] = useState([]);
    ...
    useEffect(() => {
        ...
        fetch(`http://127.0.0.1:8000/api/machine_requested/${farmer_id}/`, { method: 'GET' })
            .then(response => response.json())
            .then(data => { setMachineRequested(data); setDataCount(data.length) })
    ...
    const handleDelete = async () => {
        try {
            if (checkedRows.length > 0) {
                ...
                if (result === true) {
                    const farmer_id = localStorage.getItem('id');
                    const response = await fetch(`http://127.0.0.1:8000/api/delete_machines_requested?farmer_id=${farmer_id}`, {
                        method: 'DELETE',
                        headers: {
    ...
    const handleCheckAll = () => {
        if (checkedRows.length === machinerequested.length) { setCheckedRows([]) }
        else { setCheckedRows(machinerequested.map(item => item.id)) }
    }
    const checkAllRows = checkedRows.length === machinerequested.length;

    const showData = () => {
        const indexOfLastPage = currentPage * postsPerPage;
        const indexOfFirstPage = indexOfLastPage - postsPerPage;
        const currentLandDetails = machinerequested.slice(indexOfFirstPage, indexOfLastPage);
        try {
            return currentLandDetails.map((item, index) => (
                <tbody key={index}>
                    <tr style={{ lineHeight: "40px" }}>
                        ...
                        <td style={checkedRows.includes(item.id) ? { backgroundColor: "aliceblue" } : {}}><Link to={`/machinestatus/${item.id}`} className='text-decoration-none text-danger'><MdOutlineDescription /></Link></td>
    ...
    const showPagination = () => {
        const pageNumbers = [];
        const totalLandDetails = machinerequested.length;
            ...
            {machinerequested.length > 0 ? (
                <div className='card  text-decoration-none w-100 mx-auto m-5 ps-3 pe-3 rounded-1 shadow-lg'>
                    <div className='d-flex justify-content-between w-100 mx-auto rounded-1 border' style={{ marginTop: '-30px', backgroundColor: "orange" }}>
                        ...
                        <div className='p-3  text-light fs-5 fw-bold'>Machine Requests&nbsp;<SlEnergy className='text-dark' />{datacount}</div>
